...
const EmployeeAttendanceScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        navigation.setOptions({
            title: `${employeeName}'s Attendance`,
            headerLeft: () => (
                <TouchableOpacity
    ...
    const fetchAttendanceHistory = async () => {
        try {
            ...
            if (response.attendance && response.attendance.length > 0) {
                ...
                setSelectedLocation({
                    latitude: response.attendance[0].location.coordinates[1],
                    longitude: response.attendance[0].location.coordinates[0],
                });
            } else {
